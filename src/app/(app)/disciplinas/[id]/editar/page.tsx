...
export default function DisciplinaEditar() {
  ...
  const { data: disciplina, isLoading } = useQuery<any>({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { nome: "", descricao: "", cargaHoraria: 0, cursoId: 0, professorId: undefined, periodo: undefined, obrigatoria: true },
  });
  ...
  useEffect(() => {
    if (disciplina) {
      form.reset({
        ...
        periodo: disciplina.periodo ?? undefined,
  ...
  const update = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["disciplina", id] });
      toast({ title: "Sucesso", description: "Disciplina atualizada com sucesso." });
      router.push("/disciplinas");
    },
    onError: () => toast({ title: "Erro", description: "Não foi possível atualizar a disciplina.", variant: "destructive" }),
  });
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Editar Disciplina</h1>
          <p className="text-muted-foreground">Atualize os dados da disciplina.</p>
        </div>
      </div>
          ...
          <form onSubmit={form.handleSubmit((v) => update.mutate(v))} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField control={form.control} name="nome" render={({ field }) => (
                <FormItem className="col-span-2"><FormLabel>Nome *</FormLabel><FormControl><Input placeholder="Ex: Cálculo I" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
              <FormField control={form.control} name="descricao" render={({ field }) => (
                <FormItem className="col-span-2"><FormLabel>Descrição</FormLabel><FormControl><Input {...field} value={field.value ?? ""} /></FormControl><FormMessage /></FormItem>
              )} />
              <FormField control={form.control} name="cargaHoraria" render={({ field }) => (
                <FormItem><FormLabel>Carga Horária *</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
              <FormField control={form.control} name="periodo" render={({ field }) => (
                <FormItem><FormLabel>Período</FormLabel><FormControl><Input type="number" placeholder="Ex: 1" {...field} value={field.value ?? ""} /></FormControl><FormMessage /></FormItem>
              )} />
              ...
              <FormField control={form.control} name="obrigatoria" render={({ field }) => (
            ...
            <div className="flex justify-end gap-4 pt-4 border-t">
              <Button variant="outline" asChild><Link href="/disciplinas">Cancelar</Link></Button>
