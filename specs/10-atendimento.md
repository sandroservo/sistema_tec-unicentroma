...
| Rota | Permissão | Comportamento |
|---|---|---|
| GET `/api/pdf/boletim/[alunoId]?turmaId=` | `canAccessAluno` (admin OU próprio aluno OU responsável vinculado) | turma da query ou matrícula mais recente; médias ponderadas por disciplina (`lib/pdf/media.ts`); `Content-Disposition: inline` |
| GET `/api/pdf/declaracao/[alunoId]` | `canAccessAluno` (admin OU próprio aluno OU responsável vinculado) | declaração de matrícula com data por extenso pt-BR |
| GET `/api/pdf/historico/[alunoId]` | `canAccessAluno` (admin OU próprio aluno OU responsável vinculado) | todas as matrículas × disciplinas com média |
...
## Débitos
- ~~PDFs 403 para o próprio aluno~~ CORRIGIDO: rotas usam `canAccessAluno` de `@/lib/portal` (admin, próprio aluno ou responsável vinculado).
